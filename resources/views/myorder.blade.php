...
@section('title', 'สรุปรายการที่สั่ง - Novemnbor')
@section('content')
...
<div class="container mh-100 bg-shadow pb-md-5">
	...
	<!-- @if(isset($error))
	<input type="text" value="{{ $error }}" id="response" >
	@else
	<input type="text" value="" id="response" >
	@endif -->
	<form method="post" action="{{ URL('insertorder/data') }}" enctype="multipart/form-data" id="forminsert">
	...
	<script type="text/javascript" src="{{ URL('js/novembor/myorder.js') }}"></script>
</div>
